...
#ifdef _XBOX
	cg->refdef.x = glcfgX + (cgs.glconfig.vidWidth - cg->refdef.width)/2;
	cg->refdef.y = glcfgY + (cgs.glconfig.vidHeight - cg->refdef.height)/2;
#else
...
#ifdef _XBOX
extern qboolean gCGHasFallVector[2];
extern vec3_t gCGFallVector[2];
#else
...
static void CG_CalcIdealThirdPersonViewTarget(void)
{
	...
	{
		VectorCopy(gCGFallVector[ClientManager::ActiveClientNum()], ClientManager::ActiveClient().cameraFocusLoc);
	}
	else
	{
		VectorCopy(cg->refdef.vieworg, ClientManager::ActiveClient().cameraFocusLoc);
	}

	// Add in the new viewheight
	ClientManager::ActiveClient().cameraFocusLoc[2] += cg->snap->ps.viewheight;

	// Add in a vertical offset from the viewpoint, which puts the actual target above the head, regardless of angle.
	VectorCopy( ClientManager::ActiveClient().cameraFocusLoc, ClientManager::ActiveClient().cameraIdealTarget );
#else
	if (gCGHasFallVector)
	{
...
#ifdef _XBOX
		vertOffset = ClientManager::ActiveClient().cg_thirdPersonVertOffset;
#endif
...
#ifdef _XBOX
		ClientManager::ActiveClient().cameraIdealTarget[2] += vertOffset;
#else
...
#ifdef _XBOX
	thirdPersonRange = (float)ClientManager::ActiveClient().cg_thirdPersonRange;
#endif
...
#ifdef _XBOX
	VectorMA(ClientManager::ActiveClient().cameraIdealTarget, -(thirdPersonRange), camerafwd, ClientManager::ActiveClient().cameraIdealLoc);
#else
...
static void CG_ResetThirdPersonViewDamp(void)
{
	...
	if (ClientManager::ActiveClient().cameraFocusAngles[PITCH] > 89.0)
	{
		ClientManager::ActiveClient().cameraFocusAngles[PITCH] = 89.0;
	}
	else if (ClientManager::ActiveClient().cameraFocusAngles[PITCH] < -89.0)
	{
		ClientManager::ActiveClient().cameraFocusAngles[PITCH] = -89.0;
	}

	AngleVectors(ClientManager::ActiveClient().cameraFocusAngles, camerafwd, NULL, cameraup);
	...
	VectorCopy(ClientManager::ActiveClient().cameraIdealTarget, ClientManager::ActiveClient().cameraCurTarget);
	VectorCopy(ClientManager::ActiveClient().cameraIdealLoc, ClientManager::ActiveClient().cameraCurLoc);

	// First thing we do is trace from the first person viewpoint out to the new target location.
	CG_Trace(&trace, ClientManager::ActiveClient().cameraFocusLoc, cameramins, cameramaxs, ClientManager::ActiveClient().cameraCurTarget, cg->snap->ps.clientNum, MASK_CAMERACLIP);
	if (trace.fraction <= 1.0)
	{
		VectorCopy(trace.endpos, ClientManager::ActiveClient().cameraCurTarget);
	}

	// Now we trace from the new target location to the new view location, to make sure there is nothing in the way.
	CG_Trace(&trace, ClientManager::ActiveClient().cameraCurTarget, cameramins, cameramaxs, ClientManager::ActiveClient().cameraCurLoc, cg->snap->ps.clientNum, MASK_CAMERACLIP);
	if (trace.fraction <= 1.0)
	{
		VectorCopy(trace.endpos, ClientManager::ActiveClient().cameraCurLoc);
	}

	ClientManager::ActiveClient().cameraLastFrame = cg->time;
	ClientManager::ActiveClient().cameraLastYaw = ClientManager::ActiveClient().cameraFocusAngles[YAW];
	ClientManager::ActiveClient().cameraStiffFactor = 0.0f;
}
...
static void CG_UpdateThirdPersonTargetDamp(void)
{
	...
	else if (ClientManager::ActiveClient().cg_thirdPersonTargetDamp >=1.0||cg->thisFrameTeleport||cg->predictedPlayerState.m_iVehicleNum)
	{	// No damping.
		VectorCopy(ClientManager::ActiveClient().cameraIdealTarget, ClientManager::ActiveClient().cameraCurTarget);
	}
	else if (ClientManager::ActiveClient().cg_thirdPersonTargetDamp >= 0.0)
	{	
		// Calculate the difference from the current position to the new one.
		VectorSubtract(ClientManager::ActiveClient().cameraIdealTarget, ClientManager::ActiveClient().cameraCurTarget, targetdiff);

		// Now we calculate how much of the difference we cover in the time allotted.
		// The equation is (Damp)^(time)
		dampfactor = 1.0 - ClientManager::ActiveClient().cg_thirdPersonTargetDamp;
		dtime = (float)(cg->time-ClientManager::ActiveClient().cameraLastFrame) * (1.0/(float)CAMERA_DAMP_INTERVAL);
		...
		VectorMA(ClientManager::ActiveClient().cameraIdealTarget, -ratio, targetdiff, ClientManager::ActiveClient().cameraCurTarget);
...
static void CG_UpdateThirdPersonCameraDamp(void)
{
	...
	else if (ClientManager::ActiveClient().cg_thirdPersonCameraDamp != 0.0)
	{
		float pitch;
		float dFactor;
		...
		{
			dFactor = ClientManager::ActiveClient().cg_thirdPersonCameraDamp;
		}
		else
		{
			dFactor = 1.0f;
		...
		pitch = Q_fabs(ClientManager::ActiveClient().cameraFocusAngles[PITCH]);
		...
		if (ClientManager::ActiveClient().cameraStiffFactor > 0.0f)
		{	// The cameraStiffFactor is how much of the remaining damp below 1 should be shaved off, i.e. approach 1 as stiffening increases.
			dampfactor += (1.0-dampfactor)*ClientManager::ActiveClient().cameraStiffFactor;
		}
	}
	...
	{	
		// Calculate the difference from the current position to the new one.
		VectorSubtract(ClientManager::ActiveClient().cameraIdealLoc, ClientManager::ActiveClient().cameraCurLoc, locdiff);
		...
		// This value is how much distance is "left" from the ideal.
		VectorMA(ClientManager::ActiveClient().cameraIdealLoc, -ratio, locdiff, ClientManager::ActiveClient().cameraCurLoc);
...
#ifdef _XBOX
static void CG_OffsetThirdPersonHack(void)
{
	// only in splitscreen mode
	if(ClientManager::splitScreenMode == qfalse) 
		return;

	//Hacks to mess with vertical camera position in multiplayer.  Both
	//players needed to have their camera moved down, but player 2 needed
	//to be moved further.
	static int offset = -10;
	cg->refdef.vieworg[2] += offset;
	if(ClientManager::ActiveClientNum() == 1) {
		static int p2offset = -6;
		cg->refdef.vieworg[2] += p2offset;
	}
	
	//Make sure we don't poke through some geometry
	trace_t trace;
	CG_Trace(&trace, ClientManager::ActiveClient().cameraCurTarget, 
		cameramins, cameramaxs, 
		cg->refdef.vieworg, 
...
{
	vec3_t diff;
	float thirdPersonHorzOffset = (float)ClientManager::ActiveClient().cg_thirdPersonHorzOffset;
	...
	ClientManager::ActiveClient().cameraStiffFactor = 0.0;

	// Set camera viewing direction.
	VectorCopy( cg->refdef.viewangles, ClientManager::ActiveClient().cameraFocusAngles );
	...
	{//being held
		...
		VectorSet( ClientManager::ActiveClient().cameraFocusAngles, 0, AngleNormalize180(monster->lerpAngles[YAW]+180), 0);
	}
	else if ( cg->snap->ps.stats[STAT_HEALTH] <= 0 ) 
	{
		ClientManager::ActiveClient().cameraFocusAngles[YAW] = cg->snap->ps.stats[STAT_DEAD_YAW];
	}
	else
	{	// Add in the third Person Angle.
		ClientManager::ActiveClient().cameraFocusAngles[YAW] += ClientManager::ActiveClient().cg_thirdPersonAngle;
		{
			float pitchOffset = ClientManager::ActiveClient().cg_thirdPersonPitchOffset;
			...
			{
				float pitchPerc = ((90.0f-fabs(ClientManager::ActiveClient().cameraFocusAngles[ROLL]))/90.0f);
				ClientManager::ActiveClient().cameraFocusAngles[PITCH] += pitchOffset*pitchPerc;
				if ( ClientManager::ActiveClient().cameraFocusAngles[ROLL] > 0 )
				{
					ClientManager::ActiveClient().cameraFocusAngles[YAW] -= pitchOffset-(pitchOffset*pitchPerc);
				}
				else
				{
					ClientManager::ActiveClient().cameraFocusAngles[YAW] += pitchOffset-(pitchOffset*pitchPerc);
			...
			{
				ClientManager::ActiveClient().cameraFocusAngles[PITCH] += pitchOffset;
			}
		}
	}
	...
	if (ClientManager::ActiveClient().cameraLastFrame == 0 || ClientManager::ActiveClient().cameraLastFrame > cg->time)
	{
	...
	else
	{
		...
		{
			if (ClientManager::ActiveClient().cameraFocusAngles[PITCH] > 80.0)
			{
				ClientManager::ActiveClient().cameraFocusAngles[PITCH] = 80.0;
			}
			else if (ClientManager::ActiveClient().cameraFocusAngles[PITCH] < -80.0)
			{
				ClientManager::ActiveClient().cameraFocusAngles[PITCH] = -80.0;
			}
		}
		...
		deltayaw = fabs(ClientManager::ActiveClient().cameraFocusAngles[YAW] - ClientManager::ActiveClient().cameraLastYaw);
		if (deltayaw > 180.0f)
		{ // Normalize this angle so that it is between 0 and 180.
			deltayaw = fabs(deltayaw - 360.0f);
		}
		ClientManager::ActiveClient().cameraStiffFactor = deltayaw / (float)(cg->time-ClientManager::ActiveClient().cameraLastFrame);
		if (ClientManager::ActiveClient().cameraStiffFactor < 1.0)
		{
			ClientManager::ActiveClient().cameraStiffFactor = 0.0;
		}
		else if (ClientManager::ActiveClient().cameraStiffFactor > 2.5)
		{
			ClientManager::ActiveClient().cameraStiffFactor = 0.75;
		}
		else
		{	// 1 to 2 scales from 0.0 to 0.5
			ClientManager::ActiveClient().cameraStiffFactor = (ClientManager::ActiveClient().cameraStiffFactor-1.0f)*0.5f;
		}
		ClientManager::ActiveClient().cameraLastYaw = ClientManager::ActiveClient().cameraFocusAngles[YAW];
	...
	VectorSubtract(ClientManager::ActiveClient().cameraCurTarget, ClientManager::ActiveClient().cameraCurLoc, diff);
	{
	...
	if ( thirdPersonHorzOffset != 0.0f )
	{
		...
		VectorMA( ClientManager::ActiveClient().cameraCurLoc, thirdPersonHorzOffset, cg->refdef.viewaxis[1], ClientManager::ActiveClient().cameraCurLoc );
	}

	// ...and of course we should copy the new view location to the proper spot too.
	VectorCopy(ClientManager::ActiveClient().cameraCurLoc, cg->refdef.vieworg);

	CG_OffsetThirdPersonHack();
...
#ifdef _XBOX
	range = (float)ClientManager::ActiveClient().cg_thirdPersonRange;
	yawOffset = (float)ClientManager::ActiveClient().cg_thirdPersonAngle;
	pitchOffset = (float)ClientManager::ActiveClient().cg_thirdPersonPitchOffset;
	vertOffset = (float)ClientManager::ActiveClient().cg_thirdPersonVertOffset;
	horzOffset = (float)ClientManager::ActiveClient().cg_thirdPersonHorzOffset;
#endif
	trace_t	trace;
...
static int CG_CalcFov( void ) {
	...
	if ( cg->predictedPlayerState.pm_type == PM_INTERMISSION ) {
		...
		{ //binoculars
			if (zoomFov[ClientManager::ActiveClientNum()] > 40.0f)
			{
				zoomFov[ClientManager::ActiveClientNum()] -= cg->frametime * 0.075f;

				if (zoomFov[ClientManager::ActiveClientNum()] < 40.0f)
				{
					zoomFov[ClientManager::ActiveClientNum()] = 40.0f;
				}
				else if (zoomFov[ClientManager::ActiveClientNum()] > cgFov)
				{
					zoomFov[ClientManager::ActiveClientNum()] = cgFov;
				}
			}

			fov_x = zoomFov[ClientManager::ActiveClientNum()];
		}
		else if (cg->predictedPlayerState.zoomMode)
		{
			if (!cg->predictedPlayerState.zoomLocked)
			{
				if (zoomFov[ClientManager::ActiveClientNum()] > 50)
				{ //Now starting out at nearly half zoomed in
					zoomFov[ClientManager::ActiveClientNum()] = 50;
				}
				zoomFov[ClientManager::ActiveClientNum()] -= cg->frametime * 0.035f;//0.075f;

				if (zoomFov[ClientManager::ActiveClientNum()] < MAX_ZOOM_FOV)
				{
					zoomFov[ClientManager::ActiveClientNum()] = MAX_ZOOM_FOV;
				}
				else if (zoomFov[ClientManager::ActiveClientNum()] > cgFov)
				{
					zoomFov[ClientManager::ActiveClientNum()] = cgFov;
				...
				{	// Still zooming
#ifdef _XBOX
					static int zoomSoundTime[2] = { 0, 0 };

					if (zoomSoundTime[ClientManager::ActiveClientNum()] < cg->time || zoomSoundTime[ClientManager::ActiveClientNum()] > cg->time + 10000)
					{
						trap_S_StartSound(cg->refdef.vieworg, ENTITYNUM_WORLD, CHAN_LOCAL, cgs.media.disruptorZoomLoop);
						zoomSoundTime[ClientManager::ActiveClientNum()] = cg->time + 300;
					}
#else
			...
			if (zoomFov[ClientManager::ActiveClientNum()] < MAX_ZOOM_FOV)
			{
				zoomFov[ClientManager::ActiveClientNum()] = 50;		// hack to fix zoom during vid restart
			}
			fov_x = zoomFov[ClientManager::ActiveClientNum()];
		}
		else 
		{
			zoomFov[ClientManager::ActiveClientNum()] = 80;
	...
	{
#ifdef _XBOX
		cg->zoomSensitivity = zoomFov[ClientManager::ActiveClientNum()]/cgFov;
#else
...
static qboolean CG_ThirdPersonActionCam(void)
{
	...
	if ((cg->time - ci->saber[0].blade[0].trail[ClientManager::ActiveClientNum()].lastTime) > 300)
	{ //too long since we last got the blade position
		return qfalse;
	}

	//get direction from base to ent origin
	VectorSubtract(ci->saber[0].blade[0].trail[ClientManager::ActiveClientNum()].base, cent->lerpOrigin, positionDir);
	VectorNormalize(positionDir);

	//position the cam based on the direction and saber position
#ifdef _XBOX
	VectorMA(cent->lerpOrigin, ClientManager::ActiveClient().cg_thirdPersonRange*2, positionDir, desiredPos);
#else
...
#ifdef _XBOX
			ClientManager::ActiveClient().cg_thirdPersonAngle += cg_cameraOrbit.value;
#else
...
#ifdef _XBOX
			if(ClientManager::ActiveClient().cg_thirdPersonSpecialCam &&
#else
...
#ifdef _XBOX
	lastfov = zoomFov[ClientManager::ActiveClientNum()];
#else
	...
	else
	{
		...
		{ //zoomed/zooming in
			...
			if ( f > 1.0 ) {
				...
				fov_x = fov_x + f * ( zoomFov[ClientManager::ActiveClientNum()] - fov_x );
			}
			lastfov = fov_x;
		}
		else
		{ //zooming out
			...
			if ( f > 1.0 ) {
				...
				fov_x = zoomFov[ClientManager::ActiveClientNum()] + f * ( fov_x - zoomFov[ClientManager::ActiveClientNum()]);
			}
		}
...
#ifdef _XBOX
	if ( ClientManager::ActiveClient().shake_duration <= 0 )
#else
...
#ifdef _XBOX
	if ( cg->time > ( ClientManager::ActiveClient().shake_start + ClientManager::ActiveClient().shake_duration ) )
#else
	if ( cg->time > ( cgScreenEffects.shake_start + cgScreenEffects.shake_duration ) )
#endif
	{
#ifdef _XBOX
		ClientManager::ActiveClient().shake_intensity = 0;
		ClientManager::ActiveClient().shake_duration = 0;
		ClientManager::ActiveClient().shake_start = 0;
		return;
#else
...
#ifdef _XBOX
	intensity_scale = 1.0f - ( (float) ( cg->time - ClientManager::ActiveClient().shake_start ) / (float) ClientManager::ActiveClient().shake_duration ) * (((cgScreenEffects.FOV+cgScreenEffects.FOV2)/2.0f)/90.0f);
#else
...
#ifdef _XBOX
	intensity = ClientManager::ActiveClient().shake_intensity * intensity_scale;
#else
...
#ifdef _XBOX
	ClientManager::ActiveClient().shake_intensity = intensity;
	ClientManager::ActiveClient().shake_duration = duration;
	ClientManager::ActiveClient().shake_start = cg->time;
#else
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	if (cg->snap && ClientManager::ActiveClient().myTeam != cg->snap->ps.persistant[PERS_TEAM])
	{
		ClientManager::ActiveClient().myTeam = cg->snap->ps.persistant[PERS_TEAM];
	}
...
#ifdef _XBOX
	cg->renderingThirdPerson = ClientManager::ActiveClient().cg_thirdPerson || (cg->snap->ps.stats[STAT_HEALTH] <= 0);
#else
...
#ifdef _XBOX
	// Clear the cent->updated list, so that entities will only get updated once per frame
	if((ClientManager::splitScreenMode == qtrue && ClientManager::ActiveClientNum() == 0)
		|| (ClientManager::splitScreenMode == qfalse)) {
		for(int i = 0; i < MAX_GENTITIES; i++)
			cg_entities[i].updatedThisFrame = false;
	}
#endif
...
#ifdef _XBOX
		VectorCopy(gCGFallVector[ClientManager::ActiveClientNum()], cg->refdef.vieworg);
#else
